<template>
    ...
    <!-- ✅ 게시글이 없을 때 -->
    <div v-else-if="!loading && !post" class="flex justify-center items-center min-h-screen">
    ...
    <div v-else class="flex justify-center items-center min-h-screen">
      <div class="max-w-4xl w-full mx-auto bg-gray-100 p-6 md:p-8 rounded-lg shadow-md">
        ...
        <div class="flex items-center mt-6 text-gray-600 space-x-6">
          <span class="flex items-center">👍 좋아요: {{ post.like_count }}</span>
  ...
  export default {
    setup() {
      ...
      const newComment = ref(""); // ✅ 댓글 입력 데이터
      ...
      const submitComment = async () => {
        ...
        if (success) {
          ...
          boardStore.fetchComments(route.params.postId); // ✅ 댓글 목록 갱신
        }
      };
